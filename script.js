 document.addEventListener('DOMContentLoaded', () => {
            ...
            function updateUI(password, feedback, entropy) {
                ...
                dom.entropyLabel.textContent = `Durée estimée : ${estimateTimeToCrack(entropy)}`;
                ...
                dom.strengthBar.style.backgroundColor = level.color;
                    // Afficher le message correspondant à l'entropie
                    dom.strengthLabel.textContent = getKasperskyStrengthMessage(levelIndex);
                    dom.strengthLabel.style.color = STRENGTH_LEVELS[levelIndex].color;
            ...
            function estimateTimeToCrack(entropy) {
                ...
                if (seconds < 604800) return `${Math.ceil(seconds / 86400)} jours`;
                if (seconds < 2592000) return `${Math.ceil(seconds / 604800)} semaines`;
                if (seconds < 31536000) return `${Math.ceil(seconds / 2592000)} mois`;
                if (seconds < 315360000) return `${Math.ceil(seconds / 31536000)} années`;
                if (seconds < 3153600000) return `${Math.ceil(seconds / 315360000)} décennies`;
                return `${Math.ceil(seconds / 3153600000)} siècles`;
            }

            // --- Helper & UI Functions ---
            function generateAndSetPassword() {
                // Ouvre le panneau d'historique à chaque génération
                // Historique toujours visible, pas besoin de manipuler la classe 'hidden'
                ...
                // Ajout d'un sel basé sur le timestamp pour plus d'unicité
                const salt = Date.now() % characterPool.length;
                for (let i = 0; i < length; i++) {
                    password += characterPool[(randomValues[i] + salt + i) % characterPool.length];
                }
            ...
            function addToHistory(password) {
                // Toujours ajouter le mot de passe généré, même si identique
